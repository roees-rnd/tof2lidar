...
void chatterCallback(const LaserScan::ConstPtr &msg)
{
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        while (true)
        {
            ...
            if (msg->ranges[rev_ind(tmp_ind_wrap)]<=*max_range_ptr){
                msg_new.ranges[i] += msg->ranges[rev_ind(tmp_ind_wrap)];
                n_updated++;
            }
        ...
        if (n_updated<1.0){
            msg_new.ranges[i] = 5.007;
        }else{
            msg_new.ranges[i]/=n_updated;
        }
#ifdef USE_NORM_DIST
        msg_new.ranges[i] += (float)(msg_new.ranges[i]) * (*noise_per_dist_meter_ptr) * gauss();
#elif
        msg_new.ranges[i] += (float)(msg_new.ranges[i]) * (*noise_per_dist_meter_ptr) * (float)rand() / RAND_MAX;
#endif
    }
...
{
    ros::init(argc, argv, "lidar2tof_node");
    ros::NodeHandle n;
    std::string s;
    ...
    msg_new.angle_increment = 22.5 * DEG2RAD;
    msg_new.angle_min = 0;
    msg_new.angle_max = (16 - 1) * angInc * DEG2RAD;
    ...
    std::vector<float_t> ranges(16, 0);
...
void load_params(ros::NodeHandle &n, my_params& prm)
{
    if (n.param("lidar2tof/delta_ang_per_ray_deg", prm.delta_ang_per_ray_deg, (float)DELTA_ANG_PER_RAY_DEG))
    {
    ...
    if (n.param("lidar2tof/tof_half_fov_deg", prm.tof_half_fov_deg, (float)TOF_HALF_FOV_DEG))
    {
    ...
    if (n.param("lidar2tof/noise_per_dist_meter", prm.noise_per_dist_meter, (float)NOISE_PER_DIST_METER))
    {
    ...
    if (n.param("lidar2tof/max_range", prm.max_range, (float)MAX_RANGE))
    {
    ...
    if (n.param<std::string>("lidar2tof/frame_out", prm.frame_out, (std::string)"mr18_frame"))
    {
    ...
    if (n.param<bool>("lidar2tof/flipped", prm.flipped, false))
    {
